...
try {
	...
	$statement = $conexion->prepare('SELECT * FROM placas WHERE placa5 = :placa5');
	$statement->execute(array(':placa5' => $placaBus));

		// Comparamos si los datos ingresados son iguales a los que estan en la base de datos
	$resultado = $statement->fetch();
	if ($resultado !== false) {
		$_SESSION['placa5'] = $placaBus;
	...
	//	$consulta = "UPDATE nuevoviaje1 Set fecha_actual = NOW() WHERE placa = '$placaBus' ";
      $consulta = ("SELECT * FROM placas WHERE placa5 = '$placaBus'");

		if (mysqli_query($conexion, $consulta)){

			echo "<script> alert('Placa correcta'); </script>";
      header('Location: registroplaca5.php');
		}
	} else {
...
require 'ingresoplaca5.view.php';

...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nombreEst) or empty($cedulaEst) or empty($seccionEst)) {
      ...
      $consultaEst1 = $conexionEst->prepare('SELECT * FROM nuevoviaje7 WHERE nombre = :nombre LIMIT 1');
      $consultaEst1->execute(array(':nombre' => $nombreEst));
      $final = $consultaEst1->fetch();

      $consultaEst1 = $conexionEst->prepare('SELECT * FROM nuevoviaje7 WHERE cedula = :cedula LIMIT 1');
    ...
    if ($fallo == '') {
      $consultaEst1 = $conexionEst->prepare('INSERT INTO nuevoviaje7 (id, nombre, cedula, seccion) VALUES(NULL, :nombre, :cedula, :seccion)');
...
require 'registroplaca7.view.php';
?>

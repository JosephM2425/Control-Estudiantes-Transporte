...
try {
	...
	$statement = $conexion->prepare('SELECT * FROM placas WHERE placa6 = :placa6');
	$statement->execute(array(':placa6' => $placaBus));

		// Comparamos si los datos ingresados son iguales a los que estan en la base de datos
	$resultado = $statement->fetch();
	if ($resultado !== false) {
		$_SESSION['placa6'] = $placaBus;
	...
	//	$consulta = "UPDATE nuevoviaje1 Set fecha_actual = NOW() WHERE placa = '$placaBus' ";
      $consulta = ("SELECT * FROM placas WHERE placa6 = '$placaBus'");

		if (mysqli_query($conexion, $consulta)){

			echo "<script> alert('Placa correcta'); </script>";
      header('Location: registroplaca6.php');
		}
	} else {
...
require 'ingresoplaca6.view.php';

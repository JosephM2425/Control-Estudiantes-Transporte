...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($nombreEst) or empty($cedulaEst) or empty($seccionEst)) {
      ...
      $consultaEst1 = $conexionEst->prepare('SELECT * FROM nuevoviaje4 WHERE nombre = :nombre LIMIT 1');
      $consultaEst1->execute(array(':nombre' => $nombreEst));
      $final = $consultaEst1->fetch();

      $consultaEst1 = $conexionEst->prepare('SELECT * FROM nuevoviaje4 WHERE cedula = :cedula LIMIT 1');
    ...
    if ($fallo == '') {
      $consultaEst1 = $conexionEst->prepare('INSERT INTO nuevoviaje4 (id, nombre, cedula, seccion) VALUES(NULL, :nombre, :cedula, :seccion)');
...
require 'registroplaca4.view.php';
?>
